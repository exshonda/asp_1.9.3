...
#define INTREG_NUM            (((TMAX_INTNO - 16)/32) + 1)
#define INTREG_INDEX(intno)   ((intno - 16)/32)

/*
 *  割込み要求禁止フラグの実現のための変数
 */
extern uint32_t ief[INTREG_NUM];	/* IRQの割込み要求許可フラグの状態 */
 ...
 */
extern const uint32_t iipm_enable_irq_tbl[][INTREG_NUM];
...
x_set_ipm(PRI intpri)
{
	...
	for (i = 0; i < INTREG_NUM; i++) {
		/* 一旦全割込み禁止 */
		sil_wrw_mem((void *)((uint32_t *)NVIC_CLRENA0 + i), 0xffffffff);
		sil_wrw_mem((void *)((uint32_t *)NVIC_SETENA0 + i), (iipm_enable_irq_tbl[iipm][i] & ief[i]));
	}
	SCS_SYNC;
}
...
x_disable_int(INTNO intno)
{
	...
	if (intno == IRQNO_SYSTICK) {
		...
		tmp = intno - 16;
		sil_wrw_mem((void *)((uint32_t *)NVIC_CLRENA0 + INTREG_INDEX(intno)),
					(1 << (tmp & 0x1f)));
		ief[INTREG_INDEX(intno)] &= ~(1 << (tmp & 0x1f));
	}
	SCS_SYNC;
...
x_enable_int(INTNO intno)
{
	...
	if (intno == IRQNO_SYSTICK) {
		...
		tmp = intno - 16;
		ief[INTREG_INDEX(intno)] |= (1 << (tmp & 0x1f));
		if ((iipm_enable_irq_tbl[iipm][INTREG_INDEX(intno)] & (1 << (tmp & 0x1f))) != 0) {
			sil_wrw_mem((void *)(((uint32_t *)NVIC_SETENA0) + INTREG_INDEX(intno)),
						(1 << (tmp & 0x1f)));
		}
	}
