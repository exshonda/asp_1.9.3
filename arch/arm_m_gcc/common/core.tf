...
$  割込み優先度マスク毎の割込みを許可する割込み
$ 
const uint32_t _kernel_iipm_enable_irq_tbl[][INTREG_NUM]={$NL$
$FOREACH intpri {-4,-3,...,0}$
	$TAB${
	$start_no = 16$
	$FOREACH loop RANGE(0, (TMAX_INTNO - 16)/32)$
		$enable_mask = 0$
		$FOREACH intno RANGE(start_no, start_no+31)$
			$IF LENGTH(INT.INTNO[intno]) && (INT.INTPRI[intno] < intpri)$
 				$enable_mask = enable_mask | (1 << ((intno - 16) & 0x1f))$
			$END$
		$END$
		$start_no = start_no + 32$
		$FORMAT("UINT32_C(0x%08x),", enable_mask)$
	$END$
	$start_no = start_no + 32$
	$FORMAT("}, /* %d(%d) */", intpri+4, intpri)$$NL$
$END$
};$NL$
$NL$
